...
description: "Resumen de descargas de paquetes de rOpenSpain en CRAN y R-Universe."
---
...
options(download.file.method = if (.Platform$OS.type == "windows") {
  "wininet"
} else {
...
crandb <- rownames(available.packages(repos = Sys.getenv("R_CRAN_WEB")))


mypaks <- read_json_url(
  paste0(
...
fechas_cran <- fechas |>
  select(package, d_cran = date)

dwn <- dwn |>
  left_join(fechas_cran) |>
  filter(date >= d_cran) |>
  select(-d_cran) |>
...
# factors by avg. monthly downloads
fact <- dwn_m |>
  group_by(package) |>
  summarise(precedence = mean(downloads, na.rm = TRUE)) |>
  arrange(precedence) |>
  pull(package) |>
  unique()
...
thisw <- lubridate::ceiling_date(Sys.Date(), "week", week_start = 1)

dwn_w <- dwn |>
  filter(date >= (Sys.Date() - years(1))) |>
  mutate(m = lubridate::ceiling_date(date, "week", week_start = 1)) |>
  filter(m < thisw) |>
  group_by(package, m) |>
  summarise(downloads = sum(count))
...
# Combina las métricas y asigna nombres legibles.
end <-
  fechas |>
  left_join(avg_daily) |>
  left_join(last_day) |>
...
reactable(
  ...
  columns = list(
    ...
    "último día" = colDef(format = colFormat(separators = TRUE)),
...
names(res_all) <- c(
  "paquete", "Última versión", "en CRAN",
